...
export const advancedRuleLessons: readonly RuleLesson[] = [
  lesson({ id:'common-yaku',phaseId:'complete-yaku',order:13,title:'先把高频役认到不假思索',subtitle:'高频役正反反复',intro:'这些役构成绝大多数真实和牌，值得重复。',keyPoint:'先看能否和，再数可以叠加的役与宝牌。',points:['立直、门清自摸、断幺九、平和、役牌是日常主体。','宝牌和赤宝牌可以叠加，却仍不能单独提供和牌资格。','同一手常同时拥有多个役。'],studySections:[yakuStudySection('五个最常用的和牌资格',high,'先读成立条件，再比较自己熟悉的麻将规则哪里不同。'),doraStudy],questions:yakuRecognitionQuestions('high',high) }),
  lesson({ id:'open-yaku',phaseId:'complete-yaku',order:14,title:'吃碰以后，役还剩多少？',subtitle:'副露变化专项',intro:'这是从其他麻将切换过来最容易付出代价的地方。',keyPoint:'鸣牌前同时说出役名和副露后的番数。',points:['立直、平和、一杯口、七对子等门清限定役会消失。','三色同顺、一气通贯、混全、纯全、混一色、清一色副露减一番。','断幺九、役牌、对对和等副露不减番。'],studySections:openStudySections,questions:openChangeQuestions([...high,...common,...occasional]) }),
  lesson({ id:'regular-yaku',phaseId:'complete-yaku',order:15,title:'常见与偶尔出现的组合役',subtitle:'不强做，但要会看',intro:'这些役经常作为既有牌形的加分方向出现。',keyPoint:'认出自然路线，不为役名强拆高效牌。',points:['三色同顺与一气通贯是顺子手常见加分方向。','对对和、三暗刻、小三元常与役牌或混一色叠加。','混全与纯全会牺牲中张牌效，不应看到幺九就强做。'],studySections:[yakuStudySection('实战常见的加分役',common),yakuStudySection('偶尔出现：认识自然机会',occasional)],questions:yakuRecognitionQuestions('regular',[...common,...occasional]) }),
  lesson({ id:'rare-yaku',phaseId:'complete-yaku',order:16,title:'罕见役：见到时认识就够了',subtitle:'低频役与役满地图',intro:'不拿宝贵练习时间反复刷三色同刻，但完整规则不能留白。',keyPoint:'罕见役先会识别；役满知道成立条件，实战遇到再回看。',points:['岭上、抢杠、海底、河底依赖特殊和牌时机。','三杠子、三色同刻、二杯口等实战非常少见。','国士、四暗刻、大三元等役满不再与普通番数相加。'],studySections:[yakuStudySection('低频役与役满速查',rare,'这一页是查阅地图，不要求一次背完；先阅读一遍，再从题库随机复习。')],questions:yakuRecognitionQuestions('rare',rare),sessionSize:12,poolLabel:`${rare.length}题低频题库 · 每次抽12题` }),
  lesson({ id:'han-review',phaseId:'complete-yaku',order:17,title:'把役种按番数再记一遍',subtitle:'从役名反查番数',intro:'换一个提问方向，才能确认不是只会看关键词猜答案。',keyPoint:'1番是日常骨架；副露减番的役要成对记忆。',points:['1番役数量最多，也最常决定能不能和。','2～3番役常作为结构奖励。','混一色3/2、清一色6/5是最重要的门清/副露番差。'],studySections:hanStudySections,questions:hanReviewQuestions(),sessionSize:12,poolLabel:'全役番数题库 · 每次抽12题' }),
  lesson({ id:'route-planning',phaseId:'complete-yaku',order:18,title:'从约三向听手牌看可能的役',subtitle:'多选路线判断',intro:'不是问最后会做成什么，而是问现在有哪些自然路线值得保留。',keyPoint:'可以同时保留多个方向；进张以后再收窄，不要过早锁死。',points:['先选不需要大幅拆牌的自然役。','普通面子手与七对、国士可在早期动态比较。','“可能”不等于必须追求，牌效仍是第一层筛选。'],studySections:routeStudySections,questions:routeQuestions }),
  lesson({ id:'fu-basics',phaseId:'scoring',order:19,title:'符只学够用的部分',subtitle:'常见符数快速判断',intro:'不从罕见110符开始背；先让常见点数能算出来。',keyPoint:'先记20、25、30符三个锚点，再逐项加符并向上取整。',points:['平和自摸20符；七对子固定25符；门清荣和常从30符起。','刻子、杠子、役牌雀头与愚形等待会加符。','课程只大量练常见20～70符，题目最多一副杠。'],studySections:fuStudySections,questions:fuQuestions }),
