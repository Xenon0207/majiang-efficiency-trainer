...
面向日麻初学者的渐进式练习。首页先提供面向川麻/国标玩家的 14 课规则入门：第一阶段用六个小步骤分别介绍额外和牌条件、鸣牌与门清、立直、宝牌、舍牌振听和见逃限制，再学习六个常用役与基础防守；重点规则通过62道场景题反复练习。独立的“详细规则与算分”再用11课覆盖完整役种、副露变化、番数复习、约三向听役种路线、常见符数、300道点数题及牌桌细节；每课先提供分组讲义与速查表，再进入共411道的练习题库。1.0收录14个基础原则、每课6道静态题，共84题；2.0提供从散牌开始的连续摸切训练，同时评价普通手、七对子和国士路线。

规则与算分统一采用雀魂四人麻将口径：三张赤五、食断开启、不采用切上满贯，保留 2600 / 5200 / 7700 / 11600 等点数。
